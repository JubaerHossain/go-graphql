...
func RawInsertUpdateModel(tableName string, data map[string]interface{}, where map[string]interface{} , db *sql.DB) (int64, error) {
	// Create a slice to hold the field names and a slice to hold the field values
	var fields []string
	var values []interface{}
	...
	dsql := FindAllModel(reflect.TypeOf(data), tableName, where, fields, db)
	...
	// 			continue
	// 		}
	// 		setValues = append(setValues, fmt.Sprintf("%s = ?", field))
	// 	}
	// 	sql = fmt.Sprintf("UPDATE %s SET %s WHERE id = ?", tableName, strings.Join(setValues, ","))
